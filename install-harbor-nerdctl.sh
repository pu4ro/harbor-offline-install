...
if [ "$ENABLE_HTTPS" != "true" ]; then
    ...
    log_warn "=========================================="
    log_warn ""
    log_warn "containerd에서 insecure registry 설정:"
    log_warn ""
    log_warn "1. containerd hosts.toml 파일 생성/편집:"
    log_warn "   mkdir -p /etc/containerd/certs.d/${HARBOR_HOSTNAME}"
    log_warn "   vi /etc/containerd/certs.d/${HARBOR_HOSTNAME}/hosts.toml"
    log_warn ""
    log_warn "2. 다음 내용 추가:"
    log_warn "   server = \"http://${HARBOR_HOSTNAME}\""
    log_warn "   "
    log_warn "   [host.\"http://${HARBOR_HOSTNAME}\"]"
    log_warn "     capabilities = [\"pull\", \"resolve\", \"push\"]"
    log_warn "     skip_verify = true"
    log_warn ""
